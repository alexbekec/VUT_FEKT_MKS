...
void StartAcceleratorTask(void const * argument);
...
int main(void)
{
  ...
  osThreadDef(acceleratorTask, StartAcceleratorTask, osPriorityNormal, 0, 128);
...
/* USER CODE BEGIN Header_StartAcceleratorTask */
/**
* @brief Function implementing the acceleratorTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartAcceleratorTask */
void StartAcceleratorTask(void const * argument)
{
	/* USER CODE BEGIN StartAcceleratorTask */
	/* Infinite loop */
	...
	for(;;) {
		/*
	  int16_t vector[4] = {-5000, 0, 5000, 0};
	  for (int16_t i = 0; i < 4; i++) {
		  int16_t out = vector[i];
		  xQueueSend(xVisualQueueHandle, &out, 0);
		  osDelay(300);
	  }
		 */
	...
	/* USER CODE END StartAcceleratorTask */
}
